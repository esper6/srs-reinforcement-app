...
interface UseSynthesisOptions {
  conceptId: string;
  onSynthesisResolve?: (result: SynthesisResult) => void;
}
...
export function useSynthesis({ conceptId, onSynthesisResolve }: UseSynthesisOptions) {
  ...
  const sendMessage = useCallback(
    async (userMessage: string) => {
      ...
      try {
        ...
        if (match) {
          const result: SynthesisResult = { outcome: match[1] as SynthesisOutcome };
          setSynthesisResult(result);
          onSynthesisResolve?.(result);
        }
      } catch (err) {
